...
torch::Tensor MultiHeadAttentionImpl::forward(torch::Tensor x){
    ...
    attn_weights = torch::softmax(attn_scores / std::sqrt((double)this->head_dim), -1);  // {N,H,S,S}
...
// ----------------------------------------------------------------------
// struct{GPT3Impl}(nn::Module) -> constructor
// ----------------------------------------------------------------------
GPT3Impl::GPT3Impl(po::variables_map &vm){
...
// ----------------------------------------------------------------------
// struct{GPT3Impl}(nn::Module) -> function{forward}
// ----------------------------------------------------------------------
torch::Tensor GPT3Impl::forward(torch::Tensor x){
    ...
    x = (token_embeds + pos_embeds) * std::sqrt((double)token_embeds.size(2));
